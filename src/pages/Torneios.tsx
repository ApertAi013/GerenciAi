...
export default function Torneios() {
    ...
    try {
      ...
      setTimeout(() => {
        ...
        if (res.data?.all_revealed) {
          ...
          setTimeout(async () => {
            try {
              await tournamentService.finishLiveDraw(selectedTournament.id);
            } catch { /* bracket already generated by autoFinish */ }
            const tRes = await tournamentService.getTournament(selectedTournament.id);
            setSelectedTournament(tRes.data);
            ...
            setLiveDraw(null);
            fetchAll();
          }, 12000); // 12s delay: public page needs ~10s for spinning+reveal+fireworks
        }
      }, 3000);
    } catch (err: any) {
  ...
  const handleConfirmDraw = async () => {
    ...
    try {
      ...
      if (drawLive) {
        toast.success('Duplas sorteadas ao vivo! Animacao visivel na pagina publica.');
        // Clear live_draw_mode after 25s (enough for public page shuffle+reveal animation)
        setTimeout(async () => {
          try {
            await tournamentService.finishLiveDraw(selectedTournament.id);
